...
class UserNotesDAO extends DAOBase
{
    ...
    {
        $this->db->exec("DELETE FROM {$this->dbTable} WHERE " . self::USER_ID . " = :0 OR " . self::NOTED_ID . " = :1",
            [$id, $id]);
    }
    ...
    public function save($sequence = null)
    {
        ...
        $this->db->exec("INSERT INTO {$this->dbTable} (" . self::USER_ID . ", " . self::NOTED_ID . ", " . self::NOTE . ") VALUES " . implode(', ',
                $list));
    }
